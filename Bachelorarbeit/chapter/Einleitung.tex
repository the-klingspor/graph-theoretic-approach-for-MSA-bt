...
\begin{beispiel}
	...
	Wie man sieht haben wir zwei Übereinstimmungen (O und R), fünf Abweichungen und eine eingefügte Lücke in die erste Sequenz. Der \emph{Score} dieses Alignments beträgt also $2\cdot 1 - 2 = 0$. Nun zum zweiten:
...
\begin{lemma}
	Für zwei Sequenzen $S_1$ und $S_2$ kann der \emph{Score} eines optimalen Alignments $\mathcal{R}$ in $\oh(n\cdot m)$ Zeit und $\oh(n\cdot)$ Speicherplatz berechnet werden.
\end{lemma}
