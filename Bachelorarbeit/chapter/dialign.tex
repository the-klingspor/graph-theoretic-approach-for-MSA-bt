...
Untersucht man das Problem jedoch genauer, stellt man fest, dass man für ein \emph{Fragment} $f^{k,l}$ gar nicht alle \emph{Fragmente} auf Überlappungen überprüfen muss, sondern nur die, an denen eine der beiden Sequenzen $S_k$ oder $S_l$ unseres \emph{Fragments} beteiligt ist. Außerdem müssen gar nicht \emph{Fragmente} eines anderen paarweisen \emph{Alignments} betrachtet werden, sondern wir können in sortierten Fragmentketten durch die Start- und Endpunkte sehr genau abschätzen welche für Überlappungen in Frage kommen.
...
\begin{beweis}
	...
	Betrachten wir ein \emph{Alignment} zwischen den Sequenzen $S_i$ und $S_k$. Dann müssen wir für die \emph{Überlappgewichte} nur die Einträge $A_{i,k}$ und $A_{l,j}$ mit $1 \leq k,l \leq n$ betrachten, denn es sind nur die \emph{Alignments} relevant, bei denen eine der Sequenzen übereinstimmt. Das sind alle Listen, die in der selben Spalte oder Zeile unserer Tabelle stehen, also $\oh(n)$ viele.
