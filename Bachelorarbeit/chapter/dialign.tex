...
\begin{definition}[Alignment und Konsistenz\cite{mdw96}]\label{def:konsistenz}
...
\begin{figure}[H]
	\centering
	\includegraphics[]{chapter/trans_grenzen.png}
...
Abbildung \ref{abb:trans_grenzen} zeigt ein Beispiel für die Transitivitätsgrenzen eines Knotens in einem Graphen. Wie man sieht, entsprechen diese genau der Position des Knotens in dem Pfad zu dem er selbst gehört. Für den dritten Pfad zeigt die Vorgängergrenze tatsächlich vor das erste Element, weil es kein Element $[3,p]$ mit $[3,p] \preceq_{\mathcal{A}} e_5$ gibt. Zwischen Transitivitäts- und Konsistenzgrenzen herrscht ein direkter Zusammenhang. Ist eine Stelle $s \in \mathcal{S}$ bereits mit einer anderen Stelle aus der Sequenz $S_i \in S$ aligniert, dann gibt es zwischen allen vier Grenzen keinen Unterschied und es gilt:
