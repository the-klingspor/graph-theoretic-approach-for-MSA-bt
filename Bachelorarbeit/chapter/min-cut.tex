...
Der \emph{Inzidenzgraph} lässt sich aus den \emph{Fragmenten} der paarweisen \emph{Alignments} in $\oh(n^2\cdot)$ berechnen, weil es $\oh(n^2)$ davon gibt, die bis zu $\oh(L)$ Verbindungen enthalten.
...
Die längsten Pfade für alle Sequenzen benötigen $\oh(n\cdot L^2)$ für $n$-faches Topological Sort auf einem Graphen mit $\oh(L)$-vielen Knoten und somit $\oh(L^2)$ Kanten im schlimmsten Fall (transitive Hülle). Die topologischen Sortierungen der Knoten werden ebenfalls $n$-mal traversiert, womit eine Laufzeit von $\oh(n\cdot L)$ folgt.
